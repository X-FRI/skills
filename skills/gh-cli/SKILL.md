...
# Add multiline comment safely (recommended)
...
# Comment on PR with multiline body safely (recommended)
gh pr review 123 --comment --body-file - <<'EOF'
## Requested changes
- Fix validation
- Add regression test
EOF
...
### Multiline Body Safety (Avoid literal `\\n`)

```bash
# Recommended for gh commands that support --body-file
gh pr comment 123 --body-file - <<'EOF'
First line
Second line
EOF

# Use this for gh api body fields
...
- **MANDATORY for multiline:** Use heredoc (`<<'EOF'`) + `--body-file -` or `--raw-field body="$(cat <<'EOF' ... EOF)"`
- **NEVER** pass escaped newlines in double-quoted strings for API/comment bodies
  (for example `--raw-field body="line1\nline2"` or `--body "line1\nline2"`)
- Rationale: In shell double quotes, `\n` is literal text, not an actual newline
- If using one-line bodies only, plain `--body "..."` is acceptable

- Avoid passing escaped sequences like `\\n` in quoted strings (for example `--body "a\\nb"`)
- Use heredoc or `--body-file` to preserve real newlines

### PR Review Severity Badge Rule (MANDATORY)

- For PR review comments that include a severity level (for example `P0`, `P1`, `P2`), you **MUST** render the level with a Shields badge image from `https://img.shields.io`.
- Use static badge format: `https://img.shields.io/badge/<level>-<color>?style=flat`
- Recommended mapping: `P0 -> red`, `P1 -> orange`, `P2 -> yellow`, `P3 -> blue`
- Do not output plain severity text alone (for example `P1` without a badge) when severity is being explicitly labeled.

```markdown
**<sub><sub>![P2 Badge](https://img.shields.io/badge/P2-yellow?style=flat)</sub></sub>  Preserve default project expansion on breakpoint changes**
```
